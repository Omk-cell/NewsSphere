...
import { isArticleBookmarked, toggleBookmarkArticle } from '../../utils/bookmarks';

const NewsCard = ({ article, showBookmark = true, onBookmarkChange }) => {
  const { title, description, image, source, publishedAt, category } = article;
  const fallbackImage = 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400&h=200&fit=crop';
  const bookmarkKey = article.id || article.url || article.title;
  const bookmarked = isArticleBookmarked(article, JSON.parse(localStorage.getItem('bookmarks') || '[]'));

  const handleBookmark = (event) => {
    event.preventDefault();
    const nextBookmarks = toggleBookmarkArticle(article, JSON.parse(localStorage.getItem('bookmarks') || '[]'));
    if (onBookmarkChange) {
      onBookmarkChange(nextBookmarks);
    }
  };
    ...
    <div className="news-card group">
      <div className="relative overflow-hidden h-48">
        ...
        {showBookmark && (
          <button
            ...
            className="absolute top-3 right-3 rounded-full bg-white/90 p-2 text-lg shadow hover:scale-110 transition"
          >
            {bookmarked ? '❤️' : '🤍'}
          </button>
        )}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <Link
            to={`/article/${encodeURIComponent(bookmarkKey)}`}
